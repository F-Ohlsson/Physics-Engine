...
void DrawBox(const glm::vec3& position, const glm::quat& rotation, const float width, const float height, const float length, const glm::vec4& color, const RenderMode renderModes, const float lineWidth)
{
	...
	glm::mat4 translation = glm::translate(position) * transform;
...
void RenderLine(RenderCommand* command, Camera* cam)
{
...
void RenderBox(RenderCommand* command, Camera* cam)
{
...
void DispatchDebugDrawing(Camera* cam)
{
	while (!cmds.empty())
	{
		...
		{
			RenderLine(currentCommand, cam);
			break;
		}
		case DebugShape::BOX:
		{
			RenderBox(currentCommand, cam);
			break;
		}
		default:
		{
...
void DispatchDebugTextDrawing(Camera* cam)
{
	if (textcmds.empty())
		return;
