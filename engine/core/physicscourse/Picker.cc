...
Ray Picker::ShootRay(Display::Window* renderWindow, Camera* cam) {
	...
	glm::mat4 glmVp = cam->projection * cam->view;
	glm::mat4 glmInvVP = glm::inverse(glmVp);
	glm::vec4 screenSpaceGlm = glm::vec4(get<0>(mousePos), -1*get<1>(mousePos), -nearPlane, 1);
	
	glm::vec4 worldSpaceGlm = glmInvVP * screenSpaceGlm; //This is why GLM is used since this isn't implemented in my own mathlib
	worldSpaceGlm.w = 1.0f / worldSpaceGlm.w;
	glm::vec3 worldPos = glm::vec3(worldSpaceGlm.x * worldSpaceGlm.w, worldSpaceGlm.y * worldSpaceGlm.w, worldSpaceGlm.z * worldSpaceGlm.w);


	//mat4 transpViewMat = MathlibMat4::Transpose(cam->view);
	//vec4 fwd = transpViewMat[2] * (-1);
	//vec4 up = transpViewMat[1] * (-1);
	//vec4 right = transpViewMat[0];

	//glm::vec3 upGlm = glm::vec3(up.x, up.y, up.z);
	//glm::vec3 rightGlm = glm::vec3(right.x, right.y, right.z);
...
float Picker::IntersectsWithPhysicsObject(Ray ray, PhysicsObject* gameObj, glm::mat4 viewMatrix) {
	...
	for (int i = 0; i < 11; i++) {
		...
		float distance = ray.WillIntersectPlane(plane);


		glm::vec4 color = glm::vec4(0, 0, 1, 1);

		if (distance >= 0) {
			glm::vec3 testPoint = ray.PlaneIntersectionPoint(plane);

			if (PointInsideTriangle(triangle, testPoint)) {
	...
	for (int i = 0; i < gameObj->physN->colliderTriangles.size(); i++) {
		...
		glm::mat4 invModelMatGlm = glm::inverse((modelMatrix));

		//Translate world space ray to local space ray
		Ray localRay;
		...
		Plane plane = Plane(modelTri.vertexPos[0], modelTri.vertexPos[1], modelTri.vertexPos[2]);
  		float distance = localRay.WillIntersectPlane(plane);
		float scale = 1.f;
